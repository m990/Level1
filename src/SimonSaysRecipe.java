...
public class SimonSaysRecipe extends KeyAdapter {
 ...
 // 2. add 4 images which match keyboard keys like this: images.put(new Integer(KeyEvent.VK_UP), "image.jpg");
  	images.put(new Integer(KeyEvent.VK_M), "m.jpg");
  	images.put(new Integer(KeyEvent.VK_A), "a.jpg");
  	images.put(new Integer(KeyEvent.VK_X), "x.jpg");
  	images.put(new Integer(KeyEvent.VK_N), "n.jpg");
	...
	private void showImage() {
  	 ...
  	 //frame.add(getNextRandomImage()); //7. rename to the name of your frame
    	...
    	//Hint: use the simonSays int and a random number
     	...
     	int truth = new Random().nextInt(3);
     	if (key == 0){
     		if (truth == 0){
     		speak("Donald Trump says press the M key.");
     		}
     		else{
     			speak("Press the M key.");
     		}
     	}
     	else if (key == 1){
     		if (truth == 0){
     			speak("Donald Trump says press the A key.");
     		}
     		else{
     			speak("Press the A key.");
     		}
     	}
     	else if (key == 2){
     		if (truth == 0){
     			speak("Donald Trump says press the X key.");
     		}
     		else{
     			speak("Press the X key.");
     		}
     	}
     	else if (key == 1){
     		if (truth == 0){
     			speak("Donald Trump says press the N key.");
     		}
     		else{
     			speak("Press the N key.");
     		}
     	}
	}
